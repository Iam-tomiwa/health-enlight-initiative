  ...
  return {
    ...
    openGraph: {
      ...
      images: article.seo.image
        ? [{ url: article.seo.image, alt: article.seo.imageAlt }]
        : [],
    },
    twitter: {
          ...
          {article.body.map((block) => {
            ...
            if (block.style === "h2")
              return (
                <h2
                  key={block._key}
                  className="pt-5 font-display text-3xl font-semibold text-ink"
                >
                  {text}
                </h2>
              );
            ...
            if (block.style === "blockquote")
              return (
                <blockquote
                  key={block._key}
                  className="border-l-4 border-brand pl-6 italic text-ink"
                >
                  {text}
                </blockquote>
              );
            if (block.listItem)
              return (
                <p
                  key={block._key}
                  className="ml-5 before:mr-3 before:content-['•']"
                >
                  {text}
                </p>
              );
